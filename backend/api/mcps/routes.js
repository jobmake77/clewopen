...
import McpModel from '../../models/Mcp.js'
...
const ctrl = createResourceController(McpModel, 'mcp', 'MCP')
const { upload, uploadItem, updateItem, deleteItem } = createResourceUpload(McpModel, 'mcps', 'MCP', 'mcp')
...
router.post('/admin/batch', authenticate, authorize('admin'), auditLog('mcp_batch'), ctrl.batchAction)
router.post('/admin/:id/approve', authenticate, authorize('admin'), auditLog('mcp_approve'), ctrl.approveItem)
router.post('/admin/:id/reject', authenticate, authorize('admin'), auditLog('mcp_reject'), ctrl.rejectItem)
