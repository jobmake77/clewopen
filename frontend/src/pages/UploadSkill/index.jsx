...
const UploadSkill = () => {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await uploadSkill(formData)
      if (response.success) {
        message.success('Skill 上传成功，等待审核')
        navigate('/skills')
      }
    } catch (error) {
    ...
    <div style={{ maxWidth: 800, margin: '0 auto' }}>
      ...
      <Form form={form} layout="vertical" onFinish={handleSubmit}>
        <Form.Item label="Skill 名称" name="name" rules={[{ required: true, message: '请输入 Skill 名称' }]}>
          <Input placeholder="例如：web-search-skill" />
        </Form.Item>

        <Form.Item label="Skill 描述" name="description" rules={[{ required: true, message: '请输入 Skill 描述' }]}>
          <TextArea rows={4} placeholder="详细描述您的 Skill 功能、特点和使用场景" />
        </Form.Item>
        ...
        <Form.Item label="Skill 包文件" required extra="请上传 .zip 格式的 Skill 包，大小不超过 50MB">
