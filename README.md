...
- 分类和标签系统
- 下载和评价功能
- **在线试用 Agent**（每用户每 Agent 每日 3 次，管理员可重置 +3 次）
- 开发者主页
- 管理员审核系统（Agent 和评价）

### 平台运维与分发

- Agent 上传自动审核（`pending_auto -> pending_manual`）
- Agent 发布任务队列（异步执行、状态追踪、失败重试）
- 短期安装命令与下载 token（支持单次/限次消费）
- 管理员发布运维看板（全局任务、异常筛选、失败原因聚类、SLA 指标）
- 管理员用户管理（按用户/Agent 重置当日试用次数）
...
### Phase 4: 试用沙盒与发布运营 ✅
- ✅ 容器化试用沙盒 + 5 warm slots 预热池
- ✅ 多 Provider LLM 配置与路由
- ✅ Agent 审核-发布-安装整链路（含异步发布任务）
- ✅ 发布运维看板（告警、阈值、失败聚类、CSV 导出）
- ✅ 用户管理与每日试用配额重置
